...
ATCA_STATUS hal_i2c_send(ATCAIface iface, uint8_t word_address, uint8_t *txdata, int txlength)
{
    txdata[0] = 0x3;
    txlength++;
    if (HAL_I2C_Master_Transmit(&STNODE_BSP_sec_elm_i2c2, (uint16_t)iface->mIfaceCFG->atcai2c.slave_address, txdata, (size_t)txlength, ATCA_HAL_ATECC608A_LONG_TIMEOUT) == HAL_OK)
    {
        return ATCA_SUCCESS;
    }
    else
...
ATCA_STATUS hal_i2c_receive(ATCAIface iface, uint8_t word_address, uint8_t *rxdata, uint16_t *rxlength)
{
    ...
    {
        r = HAL_I2C_Master_Receive(&STNODE_BSP_sec_elm_i2c2, (uint16_t)iface->mIfaceCFG->atcai2c.slave_address, lengthPackage, 1, ATCA_HAL_ATECC608A_LONG_TIMEOUT);
    }
    ...
    while (--retries > 0 && r != HAL_OK)
    {
        r = HAL_I2C_Master_Receive(&STNODE_BSP_sec_elm_i2c2, (uint16_t)iface->mIfaceCFG->atcai2c.slave_address, rxdata + 1, bytesToRead, ATCA_HAL_ATECC608A_LONG_TIMEOUT);
    }
...
ATCA_STATUS hal_i2c_wake(ATCAIface iface)
{
    ...
    HAL_StatusTypeDef r = HAL_I2C_Master_Transmit(&STNODE_BSP_sec_elm_i2c2, 0x00, emptybuff, (size_t)0, ATCA_HAL_ATECC608A_LONG_TIMEOUT);
    ...
    while (--retries > 0 && r != HAL_OK)
    {
        r = HAL_I2C_Master_Receive(&STNODE_BSP_sec_elm_i2c2, (uint16_t)iface->mIfaceCFG->atcai2c.slave_address, rx_buffer, 4, ATCA_HAL_ATECC608A_LONG_TIMEOUT);
    }
...
ATCA_STATUS hal_i2c_idle(ATCAIface iface)
{
    ...
    HAL_I2C_Master_Transmit(&STNODE_BSP_sec_elm_i2c2, (uint16_t)iface->mIfaceCFG->atcai2c.slave_address, buffer, (size_t)1, ATCA_HAL_ATECC608A_LONG_TIMEOUT);
    return ATCA_SUCCESS;
}
