...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t* primitives, LoRaMacCallback_t* callbacks, LoRaMacRegion_t region )
{
    ...
    MacCtx.NvmCtx->MacParamsDefaults.SystemMaxRxError = 50;
...
LoRaMacStatus_t LoRaMacMibSetRequestConfirm( MibRequestConfirm_t* mibSet )
{
    ...
    switch( mibSet->Type )
    {
        ...
        case MIB_PUBLIC_NETWORK:
        {
            ...
            Radio.SetPublicNetwork( MacCtx.NvmCtx->PublicNetwork );
            // Required to avoid keeping the radio active after init
            Radio.Sleep( );
            break;
        }
        case MIB_RX2_CHANNEL:
...
LoRaMacStatus_t LoRaMacMcChannelSetupRxParams( AddressIdentifier_t groupID, McRxParams_t *rxParams, uint8_t *status )
{
   *status = 0x1C + ( groupID & 0x03 );
    ...
    *status &= 0xEF; // Clear the group undefined error bit
    ...
    if( RegionVerify( MacCtx.NvmCtx->Region, &verify, PHY_RX_DR ) == true )
    {
        *status &= 0xFB; // Clear the DR error bit
    }
    ...
    if( RegionVerify( MacCtx.NvmCtx->Region, &verify, PHY_FREQUENCY ) == true )
    {
        *status &= 0xF7; // Clear the Freq error bit
    }

    if( *status == ( groupID & 0x03 ) ) // No error detected
    {
        // Apply parameters
